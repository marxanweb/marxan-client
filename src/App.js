...
let BACKUP_MARXAN_SERVERS = [{
	name: 'Joint Research Centre, Italy',
	host: 'https://marxan-server-blishten.c9users.io',
	description: ''
}];
...
class App extends React.Component {
	...
	_get(params, timeout = TIMEOUT) {
		return new Promise((resolve, reject) => {
			...
			jsonp(this.requestEndpoint + params, {
				timeout: timeout
	...
	_post(method, formData, timeout = TIMEOUT, withCredentials = SEND_CREDENTIALS) {
		return new Promise((resolve, reject) => {
			...
			post(this.requestEndpoint + method, formData, {
	...
	wsMessageCallback(message) {
		...
		if (logMessage !== "" && !this.state.clumpingRunning) this.setState({
	...
	getServerCapabilities(server) {
		return new Promise((resolve, reject) => {
			...
			server = Object.assign(server, {
				httpEndpoint: "http://" + server.host + TORNADO_PATH,
				httpsEndpoint: "https://" + server.host + TORNADO_PATH,
				wsEndpoint: "ws://" + server.host + TORNADO_PATH,
				wssEndpoint: "wss://" + server.host + TORNADO_PATH,
				offline: true,
			...
			fetchJsonp(endpoint + "getServerData", {
				timeout: 2000
				...
				if (json.hasOwnProperty('info')) {
					...
					if (json.serverData.SERVER_NAME !== "") {
						server = Object.assign(server, {
							name: json.serverData.SERVER_NAME
						});
					}
	...
	setLog(message) {
		this.setState({
			streamingLog: this.state.streamingLog + message
		});
	}
	...
	processLogin() {
		this._get("getServerData").then((response) => {
			...
			this.setState({
				guestUserEnabled: response.serverData.ENABLE_GUEST_USER
			});
			//validate the use
			this.validateUser();
		});
	...
	login() {
		this._get("getUser?user=" + this.state.user).then((response) => {
			...
			//set the basemap
			var basemap = this.state.basemaps.filter(function(item) {
	...
	deleteUser(user) {
		this._get("deleteUser?user=" + user).then((response) => {
			...
			//remove the user 
			usersCopy = usersCopy.filter(function(item) {
	...
	updateUser(parameters, user = this.state.user) {
		...
		this._post("updateUserParameters", formData).then((response) => {
			...
			if (this.state.user === user) this.setState({
	...
	updateRunParams(array) {
		...
		let parameters = {};
		array.map((obj) => {
			parameters[obj.key] = obj.value; return null;
		});
	...
	initialiseInterestFeatures(oldVersion, projectFeatures) {
		...
		this.setState({
			...
			projectFeatures: outFeatures.filter(function(item) {
	...
	updateProtectedAmount(mvData) {
		...
		this.state.projectFeatures.forEach((feature) => {
			...
			let mvbestItemIndex = mvData.findIndex(function(item) {
	...
	importProject(project, description, zipFilename, files, planning_grid_name) {
		return new Promise((resolve, reject) => {
			...
			this.createImportProject(project).then((response) => {
				this.setState({
					streamingLog: this.state.streamingLog + " Project '" + project + "' created\n"
				});
				...
				this.importZippedShapefileAsPu(zipFilename, planning_grid_name, "Imported with the project '" + project + "'").then((response) => {
					...
					this.setState({
						streamingLog: this.state.streamingLog + " Planning grid imported\n"
					});
					//upload all of the files from the local system
					this.uploadFiles(files, project).then((response) => {
						this.setState({
							streamingLog: this.state.streamingLog + " All files uploaded\n"
						});
						...
						this.upgradeProject(project).then((response) => {
							this.setState({
								streamingLog: this.state.streamingLog + " Project updated to new version\n"
							});
							...
							this.updateProjectParams(project, {
								...
								this.setState({
									streamingLog: this.state.streamingLog + " Uploading planning grid to Mapbox (this may take a minute or two)"
								});
	...
	async uploadFiles(files, project) {
		...
		for (var i = 0; i < files.length; i++) {
			...
			if (file.name.slice(-4) === '.dat') {
				...
				this.setState({
					streamingLog: this.state.streamingLog + " Uploading: " + file.webkitRelativePath + "\n"
				});
	...
	loadOtherSolution(user, project, solution) {
		this.getSolution(user, project, solution).then(function(response) {
			...
			if (this.projects) {
				var _projects = this.projects.filter(function(item) {
	...
	classifyData(data, numClasses, colorCode, classification) {
		...
		if (colorCode === 'opacity') {
			...
			if ((this.state.brew.colorSchemes.opacity === undefined) || (this.state.brew.colorSchemes.opacity && !this.state.brew.colorSchemes.opacity[this.state.renderer.NUMCLASSES])) {
				...
				let newBrewColorScheme = Array(Number(this.state.renderer.NUMCLASSES)).fill("rgba(255,0,136,").map(function(item, index) {
		...
		this.setState({
			dataBreaks: this.state.brew.getBreaks()
		});
	}
	...
	mouseMove(e) {
		...
		var features = this.map.queryRenderedFeatures(e.point, {
			layers: [RESULTS_LAYER_NAME]
		});
		//see if there are any planning unit features under the mouse
		if (features.length) {
			//set the location for the popup
			if (!this.state.active_pu || (this.state.active_pu && this.state.active_pu.puid !== features[0].properties.puid)) this.setState({
					popup_point: e.point
				});
			...
			if (marxan_results) {
				...
				let marxan_results_dict = {
					"puid": vector_tile_properties.puid,
					"Number": marxan_results[0]
				};
	...
	addSpatialLayers(tileset) {
		...
		this.setState({
			resultsLayer: this.map.getLayer(RESULTS_LAYER_NAME)
		});
		...
		this.setState({
			wdpaLayer: this.map.getLayer(WDPA_LAYER_NAME)
		});
	}
	...
	changeOpacity(layerId, opacity) {
		if (this.map.getLayer(layerId)) {
			...
			//set the state
			if (layerId === RESULTS_LAYER_NAME) this.setState({
					results_layer_opacity: opacity
				});
			if (layerId === WDPA_LAYER_NAME) this.setState({
					wdpa_layer_opacity: opacity
				});
		}
	}
	...
	changeStatus(e, direction) {
		...
		var features = this.map.queryRenderedFeatures(e.point, {
			layers: [PLANNING_UNIT_LAYER_NAME]
		});
		//get the featureid
	...
	setNewFeatureDatasetDescription(description) {
		this.setState({
			featureDatasetDescription: description
		});
	}
	setNewFeatureDatasetFilename(filename) {
		this.setState({
			featureDatasetFilename: filename
		});
	}
	...
	updateFeature(feature, newProps) {
		...
		var index = features.findIndex(function(element) {
	...
	setFeaturesState(newFeatures) {
		...
		this.setState({
			...
			projectFeatures: newFeatures.filter(function(item) {
				return item.selected
			})
		});
	}
	...
	removeFeatureFromAllFeatures(feature) {
		...
		//remove the feature 
		featuresCopy = featuresCopy.filter(function(item) {
	...
	openFeatureMenu(evt, feature) {
		...
		this.setState({
			...
			puvsprLayerText: puvsprLayerText
		});
	}
	...
	toggleFeaturePuvsprLayer(feature) {
		...
		if (this.state.puvsprLayerText === HIDE_PUVSPR_LAYER_TEXT) {
			...
			//set the text
			this.setState({
				puvsprLayerText: SHOW_PUVSPR_LAYER_TEXT
			});
			...
			//set the text
			this.setState({
				puvsprLayerText: HIDE_PUVSPR_LAYER_TEXT
			});
	...
	zoomToFeature(feature) {
		...
		let nums = points.map(function(item) {
			return Number(item)
		});
		//zoom to the feature
		this.map.fitBounds([[nums[0], nums[1]], [nums[2], nums[3]]], {
			padding: 100
		});
	}
	...
	openImportPlanningGridDialog() {
		this.setState({
			importPlanningGridDialogOpen: true
		});
	}
	...
	getIntersections(iucnCategory) {
		...
		return this.protected_area_intersections.filter((item) => {
			return (iucn_categories.indexOf(item[0]) > -1);
		});
	}
	...
	runProjects(projects) {
		...
		projects.forEach((project) => {
			this.startMarxanJob("_clumping", project.projectName, false).then((response) => {
				...
				//set the state
				if (this.projectsRun === 5) this.setState({
						clumpingRunning: false
					});
			});
		});
		...
		return (
			<MuiThemeProvider>
				<React.Fragment>
					...
					<LoginDialog open={ !this.state.loggedIn } onOk={ this.processLogin.bind(this) } onCancel={ this.openRegisterDialog.bind(this) } loading={ this.state.loading } user={ this.state.user } changeUserName={ this.changeUserName.bind(this) }
						changePassword={ this.changePassword.bind(this) } password={ this.state.password } openResendPasswordDialog={ this.openResendPasswordDialog.bind(this) } marxanServers={ this.state.marxanServers } selectServer={ this.selectServer.bind(this) }
						marxanServer={ this.state.marxanServer } />
					<RegisterDialog open={ this.state.registerDialogOpen } onOk={ this.createNewUser.bind(this) } onCancel={ this.closeRegisterDialog.bind(this) } loading={ this.state.loading } />
					<ResendPasswordDialog open={ this.state.resendPasswordDialogOpen } onOk={ this.resendPassword.bind(this) } onCancel={ this.closeResendPasswordDialog.bind(this) } loading={ this.state.loading } changeEmail={ this.changeEmail.bind(this) }
						email={ this.state.resendEmail } />
					<UserMenu userMenuOpen={ this.state.userMenuOpen } user={ this.state.user } userRole={ this.state.userData.ROLE } menuAnchor={ this.state.menuAnchor } hideUserMenu={ this.hideUserMenu.bind(this) }
						openOptionsDialog={ this.openOptionsDialog.bind(this) } openProfileDialog={ this.openProfileDialog.bind(this) } logout={ this.logout.bind(this) } marxanServer={ this.state.marxanServer } />
					<HelpMenu open={ this.state.helpMenuOpen } menuAnchor={ this.state.menuAnchor } hideHelpMenu={ this.hideHelpMenu.bind(this) } openAboutDialog={ this.openAboutDialog.bind(this) } openHelpDialog={ this.openHelpDialog.bind(this) }
					/>
					<OptionsDialog open={ this.state.optionsDialogOpen } onOk={ this.closeOptionsDialog.bind(this) } onCancel={ this.closeOptionsDialog.bind(this) } loading={ this.state.loading } userData={ this.state.userData }
						saveOptions={ this.saveOptions.bind(this) } changeBasemap={ this.changeBasemap.bind(this) } basemaps={ this.state.basemaps } basemap={ this.state.basemap } />
					<UsersDialog open={ this.state.usersDialogOpen } onOk={ this.closeUsersDialog.bind(this) } onCancel={ this.closeUsersDialog.bind(this) } loading={ this.state.loading } user={ this.state.user } users={ this.state.users }
						deleteUser={ this.deleteUser.bind(this) } changeRole={ this.changeRole.bind(this) } guestUserEnabled={ this.state.guestUserEnabled } toggleEnableGuestUser={ this.toggleEnableGuestUser.bind(this) } />
					<ProfileDialog open={ this.state.profileDialogOpen } onOk={ this.closeProfileDialog.bind(this) } onCancel={ this.closeProfileDialog.bind(this) } loading={ this.state.loading } userData={ this.state.userData }
						updateUser={ this.updateUser.bind(this) } />
					<HelpDialog open={ this.state.helpDialogOpen } onOk={ this.closeHelpDialog.bind(this) } />
					<AboutDialog open={ this.state.aboutDialogOpen } onOk={ this.closeAboutDialog.bind(this) } />
					<InfoPanel open={ this.state.infoPanelOpen } activeTab={ this.state.activeTab } project={ this.state.project } metadata={ this.state.metadata } runMarxan={ this.runMarxan.bind(this) }
						stopMarxan={ this.stopMarxan.bind(this) } pid={ this.state.pid } renameProject={ this.renameProject.bind(this) } renameDescription={ this.renameDescription.bind(this) } startEditingProjectName={ this.startEditingProjectName.bind(this) }
						startEditingDescription={ this.startEditingDescription.bind(this) } editingProjectName={ this.state.editingProjectName } editingDescription={ this.state.editingDescription } features={ this.state.projectFeatures }
						project_tab_active={ this.project_tab_active.bind(this) } features_tab_active={ this.features_tab_active.bind(this) } pu_tab_active={ this.pu_tab_active.bind(this) } startPuEditSession={ this.startPuEditSession.bind(this) }
						stopPuEditSession={ this.stopPuEditSession.bind(this) } clearManualEdits={ this.clearManualEdits.bind(this) } showRunSettingsDialog={ this.showRunSettingsDialog.bind(this) } openFeatureMenu={ this.openFeatureMenu.bind(this) }
						preprocessing={ this.state.preprocessing } openFeaturesDialog={ this.openFeaturesDialog.bind(this) } changeIucnCategory={ this.changeIucnCategory.bind(this) } updateFeature={ this.updateFeature.bind(this) } userRole={ this.state.userData.ROLE }
						toggleProjectPrivacy={ this.toggleProjectPrivacy.bind(this) } />
					<ResultsPane open={ this.state.resultsPanelOpen } preprocessing={ this.state.preprocessing } solutions={ this.state.solutions } loadSolution={ this.loadSolution.bind(this) } openClassificationDialog={ this.openClassificationDialog.bind(this) }
						hideResults={ this.hideResults.bind(this) } brew={ this.state.brew } log={ this.state.streamingLog } activeResultsTab={ this.state.activeResultsTab } legend_tab_active={ this.legend_tab_active.bind(this) }
						solutions_tab_active={ this.solutions_tab_active.bind(this) } log_tab_active={ this.log_tab_active.bind(this) } clearLog={ this.clearLog.bind(this) } owner={ this.state.owner } resultsLayer={ this.state.resultsLayer }
						wdpaLayer={ this.state.wdpaLayer } pa_layer_visible={ this.state.pa_layer_visible } changeOpacity={ this.changeOpacity.bind(this) } results_layer_opacity={ this.state.results_layer_opacity } wdpa_layer_opacity={ this.state.wdpa_layer_opacity }
					/>
					<FeatureInfoDialog open={ this.state.openInfoDialogOpen } onOk={ this.closeInfoDialog.bind(this) } onCancel={ this.closeInfoDialog.bind(this) } loading={ this.state.loading } feature={ this.state.currentFeature }
						updateFeature={ this.updateFeature.bind(this) } FEATURE_PROPERTIES={ FEATURE_PROPERTIES } userRole={ this.state.userData.ROLE } />
					<Popup active_pu={ this.state.active_pu } xy={ this.state.popup_point } />
					<ProjectsDialog open={ this.state.projectsDialogOpen } onOk={ this.closeProjectsDialog.bind(this) } onCancel={ this.closeProjectsDialog.bind(this) } loading={ this.state.loading } projects={ this.state.projects }
						oldVersion={ this.state.metadata.OLDVERSION } deleteProject={ this.deleteProject.bind(this) } loadProject={ this.loadProject.bind(this) } cloneProject={ this.cloneProject.bind(this) } openNewProjectDialog={ this.openNewProjectDialog.bind(this) }
						openImportDialog={ this.openImportDialog.bind(this) } unauthorisedMethods={ this.state.unauthorisedMethods } userRole={ this.state.userData.ROLE } getAllFeatures={ this.getAllFeatures.bind(this) } />
					<NewProjectDialog open={ this.state.newProjectDialogOpen } onOk={ this.closeNewProjectDialog.bind(this) } loading={ this.state.loading } getPlanningUnitGrids={ this.getPlanningUnitGrids.bind(this) } planning_unit_grids={ this.state.planning_unit_grids }
						openFeaturesDialog={ this.openFeaturesDialog.bind(this) } features={ this.state.allFeatures } openCostsDialog={ this.openCostsDialog.bind(this) } selectedCosts={ this.state.selectedCosts } createNewProject={ this.createNewProject.bind(this) }
					/>
					<NewPlanningGridDialog open={ this.state.NewPlanningGridDialogOpen } onCancel={ this.closeNewPlanningGridDialog.bind(this) } loading={ this.state.loading } createNewPlanningUnitGrid={ this.createNewPlanningUnitGrid.bind(this) }
						countries={ this.state.countries } />
					<UploadPlanningGridDialog open={ this.state.importPlanningGridDialogOpen } onOk={ this.importPlanningUnitGrid.bind(this) } onCancel={ this.closeImportPlanningGridDialog.bind(this) } loading={ this.state.loading } requestEndpoint={ this.requestEndpoint }
						SEND_CREDENTIALS={ SEND_CREDENTIALS } checkForErrors={ this.checkForErrors.bind(this) } />
					<FeaturesDialog open={ this.state.featuresDialogOpen } onOk={ this.updateSelectedFeatures.bind(this) } onCancel={ this.closeFeaturesDialog.bind(this) } loading={ this.state.loading } metadata={ this.state.metadata }
						allFeatures={ this.state.allFeatures } deleteFeature={ this.deleteFeature.bind(this) } openNewFeatureDialog={ this.openNewFeatureDialog.bind(this) } selectAllFeatures={ this.selectAllFeatures.bind(this) }
						clearAllFeatures={ this.clearAllFeatures.bind(this) } userRole={ this.state.userData.ROLE } clickFeature={ this.clickFeature.bind(this) } addingRemovingFeatures={ this.state.addingRemovingFeatures } selectedFeatureIds={ this.state.selectedFeatureIds }
						initialiseDigitising={ this.initialiseDigitising.bind(this) } featuresDialogPopupOpen={ this.state.featuresDialogPopupOpen } closePopover={ this.closePopover.bind(this) } showNewFeaturesDialogPopover={ this.showNewFeaturesDialogPopover.bind(this) }
					/>
					<NewFeatureDialog open={ this.state.NewFeatureDialogOpen } onOk={ this.closeNewFeatureDialog.bind(this) } onCancel={ this.closeNewFeatureDialog.bind(this) } loading={ this.state.loading } setName={ this.setNewFeatureDatasetName.bind(this) }
						setDescription={ this.setNewFeatureDatasetDescription.bind(this) } setFilename={ this.setNewFeatureDatasetFilename.bind(this) } name={ this.state.featureDatasetName } description={ this.state.featureDatasetDescription } filename={ this.state.featureDatasetFilename }
						createNewFeature={ this.createNewFeature.bind(this) } checkForErrors={ this.checkForErrors.bind(this) } requestEndpoint={ this.requestEndpoint } SEND_CREDENTIALS={ SEND_CREDENTIALS }
						newFeatureSource={ this.state.newFeatureSource } />
					<PlanningGridsDialog open={ this.state.planningGridsDialogOpen } onOk={ this.closePlanningGridsDialog.bind(this) } onCancel={ this.closePlanningGridsDialog.bind(this) } loading={ this.state.loading } getPlanningUnitGrids={ this.getPlanningUnitGrids.bind(this) }
						unauthorisedMethods={ this.state.unauthorisedMethods } planningGrids={ this.state.planning_unit_grids } openNewPlanningGridDialog={ this.openNewPlanningGridDialog.bind(this) } openImportPlanningGridDialog={ this.openImportPlanningGridDialog.bind(this) }
						deletePlanningGrid={ this.deletePlanningUnitGrid.bind(this) } />
					<CostsDialog open={ this.state.CostsDialogOpen } onOk={ this.closeCostsDialog.bind(this) } onCancel={ this.closeCostsDialog.bind(this) } costs={ this.state.costs } />
					<RunSettingsDialog open={ this.state.settingsDialogOpen } onOk={ this.closeRunSettingsDialog.bind(this) } onCancel={ this.closeRunSettingsDialog.bind(this) } loading={ this.state.loading } updateRunParams={ this.updateRunParams.bind(this) }
						runParams={ this.state.runParams } showClumpingDialog={ this.showClumpingDialog.bind(this) } userRole={ this.state.userData.ROLE } />
					<ClassificationDialog open={ this.state.classificationDialogOpen } onOk={ this.closeClassificationDialog.bind(this) } onCancel={ this.closeClassificationDialog.bind(this) } loading={ this.state.loading } renderer={ this.state.renderer }
						changeColorCode={ this.changeColorCode.bind(this) } changeRenderer={ this.changeRenderer.bind(this) } changeNumClasses={ this.changeNumClasses.bind(this) } changeShowTopClasses={ this.changeShowTopClasses.bind(this) }
						summaryStats={ this.state.summaryStats } brew={ this.state.brew } dataBreaks={ this.state.dataBreaks } />
					<ClumpingDialog open={ this.state.clumpingDialogOpen } onOk={ this.hideClumpingDialog.bind(this) } onCancel={ this.hideClumpingDialog.bind(this) } tileset={ this.state.tileset } RESULTS_LAYER_NAME={ RESULTS_LAYER_NAME }
						map0_paintProperty={ this.state.map0_paintProperty } map1_paintProperty={ this.state.map1_paintProperty } map2_paintProperty={ this.state.map2_paintProperty } map3_paintProperty={ this.state.map3_paintProperty }
						map4_paintProperty={ this.state.map4_paintProperty } blmValues={ this.state.blmValues } mapCentre={ this.state.mapCentre } mapZoom={ this.state.mapZoom } rerunProjects={ this.rerunProjects.bind(this) }
						changBlmMin={ this.changBlmMin.bind(this) } changBlmMax={ this.changBlmMax.bind(this) } blmMin={ this.state.blmMin } blmMax={ this.state.blmMax } setBlmValue={ this.setBlmValue.bind(this) }
						clumpingRunning={ this.state.clumpingRunning } />
					<ImportDialog open={ this.state.importDialogOpen } onOk={ this.closeImportDialog.bind(this) } loading={ this.state.loading } requestEndpoint={ this.requestEndpoint } SEND_CREDENTIALS={ SEND_CREDENTIALS }
						importProject={ this.importProject.bind(this) } checkForErrors={ this.checkForErrors.bind(this) } setLog={ this.setLog.bind(this) } setSnackBar={ this.setSnackBar.bind(this) } user={ this.state.user }
					/>
					<Snackbar open={ this.state.snackbarOpen } message={ message } onRequestClose={ this.closeSnackbar.bind(this) } style={ { maxWidth: '800px !important' } } contentStyle={ { maxWidth: '800px !important' } }
					...
					<Popover open={ this.state.featureMenuOpen } anchorEl={ this.state.menuAnchor } onRequestClose={ this.closeFeatureMenu.bind(this) } style={ { width: '307px' } }>
						<Menu style={ { width: '207px' } } onMouseLeave={ this.closeFeatureMenu.bind(this) }>
							...
							<MenuItemWithButton leftIcon={ (this.state.currentFeature.feature_layer_loaded) ? <RemoveFromMap style={ { margin: '1px' } } /> : <AddToMap style={ { margin: '1px' } } /> } style={ { display: (this.state.currentFeature.tilesetid) ? 'block' : 'none' } } onClick={ this.toggleFeatureLayer.bind(this, this.state.currentFeature) }>
								{ (this.state.currentFeature.feature_layer_loaded) ? "Remove from map" : "Add to map" }
							</MenuItemWithButton>
							<MenuItemWithButton leftIcon={ (this.state.puvsprLayerText === HIDE_PUVSPR_LAYER_TEXT) ? <RemoveFromMap style={ { margin: '1px' } } /> : <AddToMap style={ { margin: '1px' } } /> } onClick={ this.toggleFeaturePuvsprLayer.bind(this, this.state.currentFeature) } disabled={ !(this.state.currentFeature.preprocessed && this.state.currentFeature.occurs_in_planning_grid) }>
								{ this.state.puvsprLayerText }
							</MenuItemWithButton>
					...
					<AppBar open={ this.state.loggedIn } user={ this.state.user } userRole={ this.state.userData.ROLE } infoPanelOpen={ this.state.infoPanelOpen } resultsPanelOpen={ this.state.resultsPanelOpen }
						openProjectsDialog={ this.openProjectsDialog.bind(this) } openFeaturesDialog={ this.openFeaturesDialog.bind(this) } openPlanningGridsDialog={ this.openPlanningGridsDialog.bind(this) } toggleInfoPanel={ this.toggleInfoPanel.bind(this) }
						toggleResultsPanel={ this.toggleResultsPanel.bind(this) } showUserMenu={ this.showUserMenu.bind(this) } showHelpMenu={ this.showHelpMenu.bind(this) } openUsersDialog={ this.openUsersDialog.bind(this) } />
				</React.Fragment>
			</MuiThemeProvider>
			);
	}
